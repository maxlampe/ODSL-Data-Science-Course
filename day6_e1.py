...
def metropolis(func, params, n_trials: int = 1000, prop_rng=None, x0: float = 0.):
    ...
    for i in range(n_trials):
        ...
        rho = min(func(y_curr, **params)/func(x_curr, **params), 1.)
...
fig.suptitle(f"Metropolis Gaussian Tests - {n_sim} Tterations")
x_vals = np.linspace(-4.5, 4.5, 1000)
bins = int(n_sim * 0.001)

for test_i, test in enumerate(tests):
    axs.flat[test_i].hist(test[2], bins=bins, range=[-4.5, 4.5], label="gen. RN", density=True)
